local config = {
    storage = 20000,
    pointItemId = 22118, -- Item ID, in this case is TibiaCoin just create a new one for Tournament.
    pointsPerHour = 10, -- 1 Point Per hour
...
function onlinePointsEvent.onThink(interval)
    ...
    for _, player in pairs(players) do
        ...
        if ip ~= 0 and (not config.checkDuplicateIps or not checkIp[ip]) then
            ...
            if seconds >= 300060 then
                player:setStorageValue(config.storage, 0)
                local item = player:addItem(config.pointItemId, config.pointsPerHour)
                if item then
            player:sendTextMessage(MESSAGE_INFO_DESCR, "You received 10 Tournament coin because you play for 1 minutes.")
                end
                return true
            end
